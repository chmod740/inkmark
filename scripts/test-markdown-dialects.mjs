...
test('App authenticates generated nodes before privileged preview decorators run', async () => {
  ...
  const enhancement = app.indexOf('const enhancement = enhancePreview(target, sequence', math)
  assert.ok(provenance >= 0 && provenance < decoration && decoration < math && math < enhancement)
  assert.match(app, /function enhancePreview[\s\S]*renderDiagrams\(root, sequence, renderTheme\)/)
