...
test('application shell exposes accessible tabs, one compact toolbar, sidebar footer settings, and bottom status', async () => {
  ...
  assert.match(styles, /\.app-shell \{[\s\S]*grid-template-columns: clamp\(220px, 19vw, 278px\) minmax\(0, 1fr\)/)
	...
	assert.match(app, /v-if="runtimePlatform === 'windows'"[\s\S]*class="windows-app-menubar"[\s\S]*class="windows-top-menu"[\s\S]*toggleWindowsTopMenu\('file'\)/)
	assert.match(app, /windowsTopMenu === 'file'[\s\S]*runWindowsTopMenuAction\('open-folder'\)[\s\S]*runWindowsTopMenuAction\('connect-webdav'\)[\s\S]*runWindowsTopMenuAction\('save'\)/)
	assert.match(app, /windowsTopMenu === 'file'[\s\S]*toggleWindowsRecentMenu[\s\S]*windowsRecentMenuOpen[\s\S]*openWindowsRecentItem\(item\)[\s\S]*clearRecentItems/)
	assert.match(app, /runWindowsTopMenuAction\('export-pdf'\)[\s\S]*t\('menu.export'\)[\s\S]*t\('menu.exportPDF'\)/)
	assert.match(app, /runWindowsTopMenuAction\('export-html'\)[\s\S]*t\('menu.export'\)[\s\S]*t\('menu.exportHTML'\)/)
	assert.match(app, /runWindowsTopMenuAction\('export-png'\)[\s\S]*t\('menu.export'\)[\s\S]*t\('menu.exportPNG'\)/)
	assert.match(app, /async function refreshWindowsRecentItems\(\)[\s\S]*ListRecentMenuItems[\s\S]*function openWindowsRecentItem\(item: RecentMenuItemData\)[\s\S]*openRecentItem\(item\)/)
	assert.match(app, /windowsTopMenu === 'edit'[\s\S]*runWindowsTopMenuAction\('undo'\)[\s\S]*runWindowsTopMenuAction\('paste'\)/)
	assert.match(app, /windowsTopMenu === 'view'[\s\S]*runWindowsTopMenuAction\('view-split'\)[\s\S]*runWindowsTopMenuAction\('toggle-fullscreen'\)/)
	assert.match(app, /class="windows-window-control minimise"[\s\S]*@click="WindowMinimise"/)
	assert.match(app, /class="windows-window-control maximise"[\s\S]*@click="WindowToggleMaximise"/)
	assert.match(app, /class="windows-window-control close"[\s\S]*@click="requestApplicationQuit"/)
	assert.doesNotMatch(app, /traffic-lights|window-control-style-select|windowControlStyle/)
	assert.match(styles, /\.windows-app-menubar \{[\s\S]*height: 34px;[\s\S]*--wails-draggable: drag;/)
	assert.match(styles, /\.platform-windows \.app-workbench \{ grid-template-rows: 34px 50px minmax\(0, 1fr\) 30px; \}/)
	assert.match(styles, /\.windows-top-menu \{[\s\S]*height: 34px;[\s\S]*--wails-draggable: no-drag;/)
	assert.match(styles, /\.windows-top-menu-popover \{[\s\S]*background: var\(--paper\);/)
	assert.match(styles, /\.windows-recent-menu-popover \{[\s\S]*position: absolute;[\s\S]*left: calc\(100% \+ 3px\);/)
	assert.match(styles, /\.windows-window-controls \{[\s\S]*height: 34px;[\s\S]*margin-left: auto;/)
	assert.match(styles, /\.windows-window-control \{[\s\S]*width: 46px;[\s\S]*height: 34px;[\s\S]*background: transparent;/)
	assert.match(styles, /\.windows-window-control\.close:hover \{ color: #fff; background: #c42b1c; \}/)
