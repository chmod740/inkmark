...
test('app wires pane swapping, built-in navigation, and collision-safe header CSS', async () => {
  ...
  assert.match(app, /class="source-sticky-headings"[\s\S]*'has-line-numbers': lineNumbersVisible/)
  assert.match(styles, /\.source-sticky-heading\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)/)
  assert.match(styles, /\.source-sticky-headings\.has-line-numbers \.source-sticky-heading\s*\{[^}]*grid-template-columns:\s*var\(--source-line-number-width\) minmax\(0,\s*1fr\)/)
