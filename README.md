...
墨笺是一款面向 macOS 与 Windows 的 Markdown 编辑器，提供本地文件与 WebDAV 文档编辑、实时预览、原生菜单、多种排版主题和完整的文档导出能力。编辑、渲染和导出所需资源全部随应用提供；联网功能只会在用户主动连接 WebDAV、检查更新或访问外部链接时使用。
...
- 通过“文件 → 连接 WebDAV”连接 HTTPS WebDAV 服务，在云端目录侧边栏中浏览、打开、编辑并保存 Markdown 文件；密码仅保留在当前应用进程内
...
- 连接 WebDAV、读取远端目录以及打开或保存远端 Markdown 文件
...
测试覆盖本地文件读写、WebDAV 认证与目录解析、路径编码、ETag 冲突、远端保存、受限工作区目录访问、侧边栏懒加载、最近项目、未保存文档切换与原生关闭守卫、语言设置、原生菜单、安全升级下载与安装编排、原子预览提交、版本比较与 Release 响应、导出结构、外部资源策略、同步滚动、大文档锚点上限和离线资源完整性。真实 WebDAV 回归测试仅在运行时提供 `INKMARK_WEBDAV_URL`、`INKMARK_WEBDAV_USERNAME` 与 `INKMARK_WEBDAV_PASSWORD` 时执行，并会清理临时资源。
